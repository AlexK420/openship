...
<p align="center">
  ...
  Укажите репозиторий — Openship соберет, доставит, настроит маршрутизацию и TLS-терминацию для вашего приложения. Управляйте через десктопное приложение, веб-панель или CLI.
</p>
...
|---|---|---|
| **Статический файл** (рекомендуется) | `openship completion <shell> > <path>` | Мгновенный запуск оболочки. Обновляйте после выходя новых версий CLI. |
...
Для автоматизации доступны **MCP-эндпоинт** (для ИИ-агентов) и **REST API**. В качества MCP-инструментов экспортируются только явно разрешенные маршруты с полной проверкой прав доступа. Документация доступна на [openship.io/docs](https://openship.io/docs).
